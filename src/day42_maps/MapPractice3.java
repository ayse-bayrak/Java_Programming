...
public class MapPractice3 {
    public static void main(String[] args) {
        ...
        //arrayOfMap[5] = new LinkedHashMap<>(); index number can not be more than 4

        // I want to increase second person's salary by 10000
        arrayOfMap[1].replace("salary", ((Integer)arrayOfMap[1].get("salary")) + 10000);
        System.out.println(Arrays.toString(arrayOfMap));

        // I want to change first person's name to Hasan

        arrayOfMap[0].replace("name", "Hasan");
        System.out.println("============================================================");

        // I want to count of employee whose salary is less than 1000000
        int count =0;
        for (Map<String, Object> eachMap : arrayOfMap) {
            for (Map.Entry<String, Object> eachEntry : eachMap.entrySet()) {
                if (eachEntry.getKey().equals("salary")) {
                    if (((Integer) eachEntry.getValue()) < 1000000) {
                        count++;
                    }
                }
            }
