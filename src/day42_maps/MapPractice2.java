...
public class MapPractice2 {
    public static void main(String[] args) {
            ...
            for (Map.Entry<String, Object> eachEntry : eachMap.entrySet()) {
                if(eachEntry.getKey().equals("hired_date")){
                    if(  ( (LocalDate)eachEntry.getValue() ).getYear() == 2022 ){
                        //eachEntry.getValue() even though it's local date, the reference type is object, so we can not call the Local Date methods for example getYear();
                        // Object is the parent and we are trying to call the methods from LocalDate, so we need to casted to the LocalDate, down casting
        ...
        System.out.println("---------------------------------------------------");
        //I want to replacee the first emloyee name fram Arthur to Vasil from this list.

        listOfMaps.get(0).replace("name","Vasyl");
        System.out.println(listOfMaps);

        System.out.println("---------------------------------------------------");
        // I want to increase the salary of the last person by 50000

        listOfMaps.get(0).replace("name", "Vasyl");

        // I want to increase salary of last person by 50000

        listOfMaps.get(listOfMaps.size()-1).replace("salary",  ( (Integer)listOfMaps.get(listOfMaps.size()-1).get("salary")) + 50000 );

        // I want to change the age of the second employee from 31 to 28

        listOfMaps.get(1).replace("age", 28);

        // what if I want to increase the age of second person by 4

        listOfMaps.get(1).replace("age", ((Integer)listOfMaps.get(1).get("age")) + 4 );
                                            // I need to original age

        System.out.println(listOfMaps);

        System.out.println("---------------------------------------------------");

        Map<String, Object> car1 = new LinkedHashMap<>();// this map contains one car's information.
        car1.put("brand", "BMW");
        car1.put("model", "X5");
        car1.put("year", 2021);
        car1.put("color", "Red");
